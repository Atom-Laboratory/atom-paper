...
Foram estabelecidos critérios para a avaliação das linguagens e da biblioteca, incluindo facilidade de uso, curva de aprendizado, comunidade de suporte, desempenho, compatibilidade com diferentes plataformas e recursos disponíveis para visão computacional.
...
\begin{itemize}
    ...
    \item Feedback de desenvolvedores: Coleta de opiniões e experiências de desenvolvedores que utilizaram a OpenCV em cada linguagem.
\end{itemize}
...
Os testes quantitativos foram conduzidos para medir o tempo de execução (latência) de um algoritmo clássico de detecção de objetos em imagens estáticas, utilizando a implementação Haar Cascade da biblioteca OpenCV. A escolha do algoritmo se deve à sua popularidade e presença em aplicações reais de visão computacional, como reconhecimento facial e detecção de placas veiculares.

Todo o código de teste, scripts de medição e procedimentos de pré‑processamento foram versionados e disponibilizados no repositório do projeto para garantir reprodutibilidade.
...
\hline
OpenCV (Python) & 4.11 (bindings oficiais cv2) \\
OpenCV (Java/C++) & 4.12 (Local build) \\
\label{tab:ambiente_teste}
\end{tabular}
\end{table}

\subsubsection{Seleção do Conjuntos de Dados}
Para compor o corpus de teste, foram utilizados dois conjuntos de dados públicos de referência (benchmarks) obtidos via Kaggle, selecionados pela alta variabilidade de cenários “in-the-wild”, evitando viés de dados sintéticos ou controlados em laboratório.
...
Os testes foram executados com dois modelos Haar Cascade pré-treinados, selecionados para representar diferentes níveis de complexidade computacional:
\begin{itemize}
    \item \texttt{haarcascade\_license\_plate\_rus\_16stages.xml}: Modelo com 16 estágios, priorizando velocidade.
    \item \texttt{haarcascade\_russian\_plate\_number.xml}: Modelo focado em detalhes numéricos, com maior densidade de features.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{minSize}: $30 \times 30$ pixels.
\end{itemize}

Vale ressaltar que as imagens foram processadas em sua resolução original, sem redimensionamento prévio. Essa abordagem foi adotada intencionalmente para avaliar o comportamento das linguagens frente à variabilidade de carga de memória e processamento típica de fluxos de vídeo não padronizados.
...
\begin{figure}[!ht] % [t] tenta forçar a imagem para o TOPO da coluna
    \centering
    % Ajuste a largura para 100% da coluna (ou 0.8 se quiser menor)
